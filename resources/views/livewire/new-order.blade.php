<div class="flex w-full h-fit ">
    ...
    @if (session('SUCCESS')||session('FAILURE'))
        <div class="fixed top-0 left-0 right-0 z-20 flex items-center justify-center w-full h-full">
            ...
            @if (session('SUCCESS'))
            <button class="fixed bg-indigo-100 top border-4 border-indigo-500 z-20 text-indigo-500 w-[400px] h-40 items-center justify-center flex text-center text-2xl rounded-xl mb-4"
                wire:click="ExitStatus()">
                {{ session('SUCCESS') }}
            </button>
            @else
            <button class="fixed bg-rose-100 top border-4 border-rose-500 z-30 text-rose-500 w-[400px] h-40 items-center justify-center flex text-center text-2xl rounded-xl mb-4"
                wire:click="ExitStatus()">
                {{ session('FAILURE') }}
            </button>
            @endif
        ...
        <div class="fixed bottom-0 right-0 flex items-center justify-between w-1/3 gap-4 px-4 py-4 text-white bg-gray-800">
            ...
            @else
                ...
                <div class="flex items-center gap-x-2">
                    <button wire:click="EmptyCart()"  class="px-4 py-2 rounded-lg bg-rose-500">
                        <svg class="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 7h14m-9 3v8m4-8v8M10 3h4a1 1 0 0 1 1 1v3H9V4a1 1 0 0 1 1-1ZM6 7h12v13a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V7Z"/>
                        </svg>
                    </button>
                    <button wire:click="Next()" class="px-4 py-2 bg-indigo-500 rounded-lg">NEXT</button>
                </div>
            @endif
